    ...
    , ui(new Ui::MainWindow)
{
    ...
    sqlData = new SQLData("contacts.db");
    connections();
}
...
void MainWindow::connections(){
    ...
    connect(ui->SaveToDB, &QPushButton::clicked, this, [this]() {
        if (sqlData->saveContactsToDB(manager.getContacts())) {
    ...
    connect(ui->LoadFromDB, &QPushButton::clicked, this, [this]() {
        std::vector<Contact> contacts = sqlData->loadContactsFromDB();
        for (const auto& contact : contacts) {
            manager.addContact(contact);
        }
        FileHandler::LoadContactsToTable(manager.getContacts(), ui->tableWidget);
